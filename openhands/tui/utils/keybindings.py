...
class KeyBindings:
    ...
    def cycle_active_panel(self, forward: bool = True) -> None:
        ...
        logger.debug(
            f"Switched active panel from {self.app.active_panel} to {new_panel}"
        )
...
"""
    ...
    def handle_enter(self) -> bool:
        ...
        if self.app.active_panel == "chat":
            ...
            try:
                if (
                    hasattr(self.app.chat_panel, "input_field")
                    and hasattr(self.app.chat_panel.input_field, "is_focused")
                    and self.app.chat_panel.input_field.is_focused
                ):
