...
class TUIEventManager:
    ...
    def _handle_cmd_output_observation(self, session_id: str, event: CmdOutputObservation) -> None:
    ...
    def _handle_file_edit_observation(self, session_id: str, event: FileEditObservation) -> None:
    ...
    def _handle_file_read_observation(self, session_id: str, event: FileReadObservation) -> None:
        """Handle FileReadObservation events (file reads)."""
        file_info = f"📖 File read: {event.path}\n"
        self.file_manager.update_logs_file(session_id, file_info)

    def _handle_agent_state_changed(self, session_id: str, event: AgentStateChangedObservation) -> None:
    ...
    def send_agent_state_change(self, session_id: str, new_state: AgentState) -> bool:
        ...
        try:
            ...
            logger.info(f"Sent state change to {new_state.value} for session {session_id}")
            return True
