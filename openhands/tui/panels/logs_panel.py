...
class TUILogHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        try:
            ...
            if (
                record.name.startswith("openhands")
                and "session" in record.getMessage().lower()
            ):
                print(
                    f"[TUI LOG CAPTURE] {record.levelname}: {record.getMessage()}",
                    flush=True,
                )
...
class LogsPanel(BasePanel):
    ...
    def update_logs_display(self, session_id: str = "") -> None:
        ...
        logs_display_container_height = max(
            1, self.height - 2
        )  # Height of the logs_display container itself
    ...
    def get_session_logs(self, session_id: str) -> list[dict]:
        ...
        for log_entry in logs:
            log_level_num = getattr(
                logging, log_entry.get("level", "INFO"), logging.INFO
            )
    ...
    def create_log_widget(self, log_entry: dict) -> ptg.Widget:
        ...
        prefix_markup = (
            f"[{color}]{timestamp_str} {level}[/{color}] "  # Markup for display
        )

        prefix_visual_len = len(
            ptg.strip_markup(prefix_markup)
        )  # Actually, just len(prefix_text_raw) is fine here
