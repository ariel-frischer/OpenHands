...
class SessionsPanel(BasePanel):
    ...
    def __init__(self, session_manager: "SessionManager", file_manager: "TUIPanelFileManager"):
        ...
        logger.debug(f"SessionsPanel initialized with dimensions: {getattr(self, 'width', 'unknown')}x{getattr(self, 'height', 'unknown')}")
    ...
    def create_session_widget(self, session: "SessionContext") -> ptg.Widget:
        ...
        button = ptg.Button(
            label,
            lambda s=session.sid: self.switch_to_session_sync(s)
        )
    ...
    def create_new_session(self, *args) -> None:
        ...
        try:
            ...
            try:
                ...
                async_task = asyncio.create_task(self._create_session_async(task_description))
    ...
    async def _create_session_async(self, task: str) -> None:
        ...
        try:
            ...
            if "docker" in error_msg:
                logger.error("Docker-related error. Please ensure Docker is running and accessible.")
            elif "network" in error_msg or "connection" in error_msg:
                logger.error("Network connectivity issue. Please check your internet connection.")
            elif "permission" in error_msg:
                logger.error("Permission error. Please check file/directory permissions.")
            else:
