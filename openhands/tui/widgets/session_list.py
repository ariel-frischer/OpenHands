...
class SessionList(ptg.Container):
    ...
    def create_session_widget(self, session: "SessionContext") -> ptg.Widget:
        ...
        status = self.format_session_status(
            session.agent_state.value if session.agent_state else "unknown"
        )
        ...
        button = ptg.Button(label_text, lambda s=session.sid: self.select_session(s))
